    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent className="w-[540px] !max-w-none overflow-y-auto">
        <SheetHeader>
          <SheetTitle>
            {isEditMode
              ? `Editar Turma: ${identifier}`
              : "Adicionar Nova Turma"}
          </SheetTitle>
          <SheetDescription>
            {isEditMode
              ? "Edite os detalhes da turma aqui. Clique em salvar quando terminar."
              : "Adicione os detalhes da nova turma aqui. Clique em salvar quando terminar."}
          </SheetDescription>
        </SheetHeader>
        <div className="grid gap-6 py-4">
            ...
            <Select>
              ...
              <SelectContent>
                <SelectGroup>
                  {SALAS.map((turma) => (
                    <SelectItem key={turma.key} value={turma.key}>
                      {turma.label} - Capacidade ({turma.capacidade} alunos)
                    </SelectItem>
                  ))}
                </SelectGroup>
              </SelectContent>
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="de">Número máximo de estudantes</Label>
            <Input
              placeholder="0"
              type="number"
              id="nest"
