...
import { EyeIcon, ChevronDownIcon, SearchIcon } from "@/components/ui/icon";


import { DeleteModal } from "../modal/delete";

import { turmasD } from "@/constants/academic";
...
export const statusOptions = [
  { name: "Publicadas", uid: "Pago" },
  { name: "Em atraso", uid: "Em atraso" },
  { name: "Pendentes", uid: "Pendentes" },
];
...
export default function TableTurmas() {
  ...
  const filteredItems = React.useMemo(() => {
    let filteredUsers = [...turmasD];

    if (hasSearchFilter) {
      filteredUsers = filteredUsers.filter((est) =>
        est.trm.toLowerCase().includes(filterValue.toLowerCase())
      );
    }
    if (
      statusFilter !== "all" &&
  ...
  }, [turmasD, filterValue, statusFilter]);
            ...
            <div className="relative flex items-center gap-2 text-center">
              <Tooltip content="Visualizar">
                <span
                  ...
                  onClick={() => router.push(`notas/${est.trm}`)}
                >
                  <EyeIcon />
                </span>
              </Tooltip>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex justify-between items-center">
          <span className="text-default-400 text-small">
            {turmasD.length} Estudantes no total
          </span>
