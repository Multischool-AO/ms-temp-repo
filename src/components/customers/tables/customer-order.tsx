...
const columns = [
  { name: "Número de estudante", uid: "oid" },
...
export default function CustomerOrder({ customerId }: { customerId: string }) {
        ...
        case "update_status":
          return (
            <Select
              ...
              size="sm"
            >
              {["pendente", "ongoing", "delivered"].map((value) => (
                <SelectItem
                  key={value}
                  value={value}
                  className="capitalize"
  ...
  return (
    <>
      ...
      <h1 className="mt-7 text-xl font-medium text-zinc-400">
        Matrículas recentes
      </h1>
      <Table
      ...
      >
        ...
        <TableBody
          emptyContent={"Sem matrículas recentes"}
          items={orders?.orders || []}
        >
          {(item) => (
