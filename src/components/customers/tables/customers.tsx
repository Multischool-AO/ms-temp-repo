...
const columns = [
  { name: "Nº MATRÍCULA", uid: "id" },
  { name: "NOME", uid: "name", sortable: true },
  { name: "EMAIL", uid: "email" },
  { name: "SEXO", uid: "gender" },
  { name: "TELEFONE", uid: "phone" },
  { name: "INSCRITO EM", uid: "createdAt", sortable: true },
  { name: "CURSO", uid: "updatedAt", sortable: true },
  { name: "TURMA", uid: "lastLogin", sortable: true },
...
export default function Customers() {
  ...
  const filteredItems = React.useMemo(() => {
    let filteredUsers = Array.isArray(customers?.customers)
      ? [...customers.customers]
      : [];
  ...
  }, [filterValue, hasSearchFilter, customers]);
      ...
      <div className="mt-5 flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between">
          <span className="text-small text-default-400">
            Total de estudantes: {customers?.customers.length}
          </span>
  ...
  return (
    ...
    >
      ...
      <TableBody
        emptyContent={"Nenhum estudante encontrado"}
        items={sortedItems}
      >
        {(item) => (
